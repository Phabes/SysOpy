...
void receive_signal(int signal_number){
    received++;
    printf("RECEIVED: %d, SENDED: %d\n", received, sended);
    if(received < number_of_signals)
        next();
    else
        end();
}
...
void receive_signal_sigqueue(int signal_number, siginfo_t *info, void *ucontext){
    received++;
    printf("RECEIVED: %d, SENDED: %d, VALUE: %d\n", received, sended, info->si_value.sival_int);
    if(received < number_of_signals)
        next();
    else
        end();
